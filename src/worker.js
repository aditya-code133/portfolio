...
export default {
  async fetch(request, env, ctx) {
        ...
        const emailHtml = `
          ...
          <body>
            <div class="container">
              ...
              <div class="footer">
                Dispatched from Aditya Swain Portfolio Beacon • ${new Date().toUTCString()}
              </div>
            </div>
          </body>
          </html>
    ...
    if (url.pathname === '/api/github-stats') {
      ...
      try {
        const ghHeaders = {
          'User-Agent': 'Aditya-Portfolio-Cloudflare-Worker/1.0',
          Accept: 'application/vnd.github.v3+json',
        };

        const [userRes, reposRes] = await Promise.all([
          fetch('https://api.github.com/users/aditya-code133', { headers: ghHeaders }),
          fetch('https://api.github.com/users/aditya-code133/repos?sort=pushed&per_page=1', {
            headers: ghHeaders,
          }),
        ]);
